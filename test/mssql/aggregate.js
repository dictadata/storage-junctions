...
async function tests() {
  ...
  await retrieve({
    ...
    terminal: {
      output: './test/output/mssql_aggregate_1.json'
    }
  });

  logger.info("=== mssql groupby with summary");
  await retrieve({
    origin: {
      smt: "mssql|server=localhost;userName=dicta;password=data;database=storage_node|foo_schema|*",
      pattern: {
    ...
    terminal: {
      output: './test/output/mssql_aggregate_2.json'
    }
  });
  ...
  await retrieve({
    ...
    terminal: {
      output: './test/output/mssql_aggregate_3.json'
    }
  });
