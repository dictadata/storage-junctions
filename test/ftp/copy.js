...
async function test_1() {
  ...
  if (await getFiles({
    origin: {
      smt: "*|ftp://dicta:data@127.0.0.1/data/dictadata.org/test/input/|*.csv|*",
      options: {
        recursive: false
      }
    },
...
async function test_2() {
  ...
  if (await putFiles({
    ...
    terminal: {
      smt: "*|ftp://dicta:data@127.0.0.1/data/dictadata.org/test/output/uploads/|*|*",
      options: {}
    }
  })) return 1;
}
...
async function test_3() {
  ...
  if (await getFiles({
    origin: {
      smt: "*|ftp://dicta:data@127.0.0.1/data/US/IA/sos.iowa.gov/shapefiles/City Precincts/|Iowa*.zip|*",
      options: {
        recursive: false
      }
    },
