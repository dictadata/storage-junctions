...
class OracleDBJunction extends StorageJunction {
    ...
    try {
      ...
      for (let column of results.rows) {
        if (column["CONSTRAINT_TYPE"] === "P") {
          ...
          if (!hasOwnProperty(this.engram.indices, column["INDEX_NAME"])) this.engram.indices[column["INDEX_NAME"]] = {};
          let index = this.engram.indices[column["INDEX_NAME"]];
          index.unique = (column["UNIQUENESS"] === "UNIQUE");
          index[column["COLUMN_POSITION"] - 1] = {
  ...
  async putEncoding(encoding, overlay=false) {
    ...
    try {
      ...
      // create table
      sql = sqlEncoder.sqlCreateTable(engram, options);
      logger.verbose(sql);
      connection = await this.pool.getConnection();
      results = await connection.execute(sql);
      this.engram.replace(encoding);
  ...
  async _request(sql) {
    ...
    try {
      connection = await this.pool.getConnection();
      let results = await connection.execute(sql, {}, { autoCommit: true });
