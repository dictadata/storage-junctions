...
*/
      // get column metadata
      let sql = sqlEncoder.sqlDescribeTable(this.engram.schema);     
      connection = await this.pool.getConnection();
      let results = await connection.execute(sql, {
        resultSet: true,
      ...
      for (let row of results.rows) {
        let field = this.engram.find(row["TABLE_NAME"]);
        field["isKey"] = true;
      }      
  ...
  async putEncoding(encoding, overlay=false) {
    ...
    try {
      ...
      let sql = "SHOW FULL TABLES LIKE '" + this.smt.schema + "';";
      connection = await this.pool.getConnection();
      let results = await connection.execute(sql);
      let tables = results.rows;
  ...
  async store(construct, pattern) {
    ...
    try {
      ...
      return new StorageResults((results.rowsAffected > 0) ? "ok" : "not stored", null, null, this.options.meta ? results : null);
    }
    catch (err) {
