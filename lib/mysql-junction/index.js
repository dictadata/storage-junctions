...
class MySQLJunction extends StorageJunction {
    ...
    try {
      ...
      for (let column of columns) {
        if (column["Key_name"] === "PRIMARY") {
          ...
          if (!hasOwnProperyt(this.engram.indices, column["Key_name"])) this.engram.indices[column["Key_name"]] = {};
          let index = this.engram.indices[column["Key_name"]];
          index.unique = !column["Non_unique"];
          index[column["Seq_in_index"] - 1] = {
  ...
  async putEncoding(encoding, overlay=false) {
    ...
    try {
      ...
      // create table
      let sql2 = sqlEncoder.sqlCreateTable(engram, this.options.bulkLoad);
      logger.verbose(sql2);
