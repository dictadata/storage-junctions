...
module.exports = class CsvWriter extends StorageWriter {
  ...
  _write(construct, encoding, callback) {
    ...
    try {
      ...
      var data = '';
      var first = true;
      for (var i = 0; i < this._encoding.fields.length; i++) {
        if (first)
          first = false;
        else
          data += ',';

        if (this._encoding.fields[i].name) {
          let value = construct[this._encoding.fields[i].name];
          if (value !== null) {
            switch (this._encoding.fields[i].type) {
            case "boolean":
