...
class StorageJunction {
  ...
  async dullEncoding(options) {
    logger.verbose('StorageJunction dullEncoding');
    options = Object.assign({}, this.options, options);
    let schema = options.schema || this.smt.schema;

    // junctions that don't use filesystems should override the dullEncoding() method
    if (!Cortex.FileSystems.isUsedBy(this.smt.model))
      throw new StorageError({ statusCode: 501 }, "StorageJunction.dullEncoding method not implemented");
