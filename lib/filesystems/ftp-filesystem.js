  ...
  */
  async createWriteStream(options) {
    ...
    try {
      ...
      // create the read stream
      await this.walkCWD(this._url.pathname);
  ...
  async upload(options) {
    ...
    try {
      let src = path.join(options.downloads, options.rpath);
  ...
  async walkCWD(path) {
    ...
    for (let dir of elements) {
      rc = await this.cwd(dir);
      if (rc !== 0) {
        rc = await this.mkdir(dir);
        if (rc === 0)
          rc = await this.cwd(dir);
      }
