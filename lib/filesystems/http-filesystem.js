...
module.exports = exports = class httpFileSystem extends FileSystem {
  ...
  async list(options) {
    ...
    try {
      ...
      async function scanner(dirpath) {
        ...
        let content = await that.getHttp(dirpath);
        //console.log(content);
  ...
  */
  ...
  async getHttp(rpath, options) {
    if (this.options.http === 2)
      return this.getHTTP2r(rpath, options);
    else
      return this.getHTTP1(rpath, options);
  }

  async getHTTP1(rpath, options) {
    this.response = {};
