...
module.exports = class JsonJunction extends StorageJunction {
  ...
  constructor(storagePath, options = null) {
    ...
    this.filename = this._options.filename || path.join(this._encoding.location, this._encoding.schema) || '';
  }
  ...
  async dull(options = null) {
    try {
      ...
      try {
        //if (fs.existsSync(this._encoding.schema))
        //  fs.truncateSync(this._encoding.schema);
      }
      catch(err) {
        if (err.status != 404)
          console.log(err);
