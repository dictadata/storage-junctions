...
module.exports = exports = class XlsxJunction extends StorageJunction {
  ...
  constructor(SMT, options) {
    ...
    this.workbookDefaults = {
      cellDates: true
    };
    this.workbookOptions = Object.assign(this.workbookDefaults, this.options.xlsx.workbook);
  }
  ...
  async activate() {
    ...
    if (fs.existsSync(this.filepath) && !this.options.xlsx.overwrite) {
      logger.debug("load file " + this.filepath);
      this.workbook = XLSX.readFile(this.filepath, this.workbookOptions);
    }
    else {
  ...
  async relax() {
    ...
    // save file
    XLSX.writeFile(this.workbook, this.filepath, this.workbookOptions);
  }
  ...
  async getEncoding() {
    ...
    try {
      ...
      if (!this.engram.defined) {
        ...
        let codify = this.getTransform('codify', this.options.codify || {});
