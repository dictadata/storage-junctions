...
class MSSQLJunction extends StorageJunction {
  ...
  async activate() {
    return new Promise(async (resolve, reject) => {
      ...
      let bulkLoad = this.options.bulkLoad;
  ...
  async getEncoding() {
    ...
    try {
      ...
      await this._request(sql, (columns) => {
        for (let column of columns) {
          if (column["is_primary_key"]) {
            let field = this.engram.find(column["column_name"]);
            field.keyOrdinal = column["key_ordinal"];
          }
          else {
            if (!hasOwnProperty(this.engram, "indices")) this.engram.indices = {};
            if (!hasOwnProperyt(this.engram.indices, column["index_name"])) this.engram.indices[column["index_name"]] = {};
            let index = this.engram.indices[column["index_name"]];
            index.unique = column["is_unique"];
            index[column["key_ordinal"] - 1] = {
              "name": column["column_name"],
              "order": column["is_descending_key"] ? "DESC" : "ASC"
            }
          }
        }
      });
  ...
  async putEncoding(encoding, overlay=false) {
    ...
    try {
      ...
      let sql = sqlEncoder.sqlCreateTable(engram, this.options.bulkLoad);
      logger.verbose(sql);
  ...
  async recall(pattern) {
    ...
    try {
      ...
      let rowCount = this._request(sql, (row) => {
