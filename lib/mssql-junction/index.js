  ...
  async getEncoding() {
    ...
    try {
      ...
      await this._request(sql, (column) => {
        ...
        if (hasOwnProperty(column, "is_primary_key")) {
          // primary key index
          let field = this.engram.find(column["column_name"].value);
          field.keyOrdinal = column["key_ordinal"].value;
        }
        else {
          // other index
          if (!hasOwnProperty(this.engram, "indices")) this.engram.indices = {};
          if (!hasOwnProperty(this.engram.indices, index_name)) this.engram.indices[index_name] = {};
          let index = this.engram.indices[index_name];
          index.unique = column["is_unique"];
          index.fields[column["key_ordinal"] - 1] = {
