...
module.exports = class MySQLJunction extends StorageJunction {
  ...
  async relax() {
    logger.debug("mysqlJunction relase");
  ...
  async putEncoding(encoding) {
    ...
    try {
      ...
      // create table
      let sql2 = encoder.sqlCreateTable(engram);
  ...
  async recall(options = null) {
    ...
    try {
      ...
      let sql = "SELECT * FROM " + this._engram.smt.schema + encoder.sqlWhereFromKey(this._engram, options);
  ...
  async dull(options = null) {
    ...
    try {
      ...
      if (this._engram.keyof === 'list' || this._engram.keyof === 'all') {
        ...
        let sql = "DELETE FROM " + this._engram.smt.schema + encoder.sqlWhereFromKey(this._engram, options);
