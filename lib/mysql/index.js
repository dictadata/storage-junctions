...
module.exports = class MySQLJunction extends StorageJunction {
  ...
  async store(construct, options = null) {
    ...
    try {
      if (Object.keys(this._engram.encoding.fields).length == 0)
  ...
  async recall(options = null) {
    if (this._engram.keyof === 'uid') {
      throw "uid key not supported";
    }

    try {
      let sql = encoder.sqlSelectWithKey(this._engram, options);
      let results = await this.pool.query(sql);
  ...
  async retrieve(pattern, options = null) {
    ...
    try {
      ...
      let sql = encoder.sqlSelectWithPattern(this._engram, pattern, options);
      let results = await this.pool.query(sql);
      //console.log('The solution is: ', results[0].solution);
