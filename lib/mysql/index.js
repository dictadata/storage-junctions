...
module.exports = class MySQLJunction extends StorageJunction {
  ...
  async getEncoding() {
    try {
      ...
      let columns = await this.pool.query("DESCRIBE " + this._encoding.schema + ";");
  ...
  async putEncoding(encoding) {
    try {
      ...
      let tables = await this.pool.query("SHOW FULL TABLES LIKE '" + this._encoding.schema + "';");

      if (tables.length === 0) {
        // create table
        await this.pool.query(encoder.createTable(this._encoding));
      }
  ...
  async store(construct, options = null) {
    ...
    try {
      await this.pool.query(encoder.createInsert(this._encoding, construct));
