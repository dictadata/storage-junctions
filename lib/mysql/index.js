...
module.exports = class MySQLJunction extends StorageJunction {
  ...
  constructor(SMT, options = null) {
    ...
    let pairs = this._engram.smt.locus.split(';');
  ...
  async getEncoding() {
    ...
    try {
      ...
      let sql = "DESCRIBE " + this._engram.smt.schema + ";";
  ...
  async putEncoding(encoding) {
    ...
    try {
      ...
      let sql1 = "SHOW FULL TABLES LIKE '" + this._engram.smt.schema + "';";
  ...
  async store(construct, pattern=null) {
    ...
    try {
      ...
      return new StorageResults( (results.affectedRows > 0) ? "ok" : "not stored", null, null, this._options.meta ? results : null);
    }
    catch(err) {
  ...
  async recall(pattern=null) {
    ...
    try {
      ...
      let sql = "SELECT * FROM " + this._engram.smt.schema + sqlQuery.sqlWhereFromKey(this._engram, pattern);
  ...
  async dull(pattern=null) {
    ...
    try {
      ...
      if (this._engram.keyof === 'list' || this._engram.keyof === 'all') {
        ...
        let sql = "DELETE FROM " + this._engram.smt.schema + sqlQuery.sqlWhereFromKey(this._engram, pattern);
      ...
      return new StorageResults((results.affectedRows > 0) ? "ok" : "not found", null, null, (this._options.meta ? results : null));
    }
    catch (err) {
      this._logger.error(err.message);
      throw err;
