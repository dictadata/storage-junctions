...
module.exports = class MySQLJunction extends StorageJunction {
  ...
  async getEncoding() {
    try {
      ...
      let columns = await this.pool.query("DESCRIBE " + this._engram.schema + ";");

      for (let i = 0; i < columns.length; i++) {
        let field = encoder.storageField(columns[i]);
        this._engram.encoding.add(field);
      }
  ...
  async putEncoding(encoding) {
    ...
    try {
      this._engram.encoding.merge(encoding);

      // check if table already exists
      let tables = await this.pool.query("SHOW FULL TABLES LIKE '" + this._engram.schema + "';");

      if (tables.length === 0) {
        // create table
        await this.pool.query(encoder.createTable(this._engram));
      }
  ...
  async store(construct, options = null) {
    ...
    try {
      await this.pool.query(encoder.createInsert(this._engram, construct));
  ...
  async recall(options = null) {
    ...
    try {
      let results = await this.pool.query('SELECT 1 + 1 AS solution');
