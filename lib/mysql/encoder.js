...
exports.sqlWhereFromKey = function (engram, options) {
  ...
  if (engram.keys.length > 0) {
    ...
    for (let key of engram.keys) {
      ...
      if (types.includes(engram.fields[key].type))
        value = "'" + value + "'";
 ...
 */
exports.sqlSelectWithPattern = function (engram, options) {

  let pattern = options && options.pattern;

  let sql = "SELECT * FROM " + engram.schema;
  ...
  if (pattern && pattern.filter) {
    ...
    for (let name in pattern.filter) {
      ...
      // check if value needs to be quoted
      let types = ['text', 'keyword', 'date'];
      if (types.includes(engram.fields[name].type))
        value = "'" + value + "'";
