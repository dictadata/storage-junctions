...
module.exports = exports = class FtpFileSystem extends StorageFileSystem {
  ...
  constructor(SMT, options) {
    ...
    this._ftp._delete = util.promisify(this._ftp.delete);
  ...
  connect(options) {
    return new Promise((resolve, reject) => {
      ...
      this._ftp.on('error', (err) => {
        logger.error('ftp event error' + JSON.stringify(that._url) + that.smt.schema);
        reject(err);
      });
  ...
  */
  ...
  async _walkCWD(path) {
    ...
    for (let dir of elements) {
      ...
      if (rc !== 0)
        throw new Error("Could not walk ftp directory: " + rc);
    }
  }
