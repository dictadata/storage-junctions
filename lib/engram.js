...
module.exports = exports = class Engram {
  ...
  get_uid(construct) {
    if (this.keyof === 'uid') {
      ...
      // generate an uid from key or primary fields
      let uid = '';
      for (let kname of this.keys) {
        if (this.caseInsensitive) {
          let cnames = Object.keys(construct);
          for (let cname of cnames) {
            if (cname.toUpperCase() === kname)
              uid += construct[cname];
          }
        }
        else if (construct[kname])
          uid += construct[kname];  //.toString();
      }
      return uid ? uid : null;
    }
    //return null;
  ...
  find(name) {
    ...
    if (!field) {
      field = new Field({
        name: name,
        isKey: this.keys.includes(fname)
      });
    }
    return field;
  }
  ...
  add(field) {
    ...
    if (this.keys.includes(fname))
      newField.isKey = true;
    this.fields[fname] = newField;
  ...
  merge(encoding) {
    ...
    for (let [fname, field] of Object.entries(newFields)) {
      let name = field.name || fname;
