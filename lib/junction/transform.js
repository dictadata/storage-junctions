...
module.exports = exports = class StorageTransform extends Transform {
  ...
  constructor(storageJunction, options = null) {
    ...
    this._options = Object.assign({}, storageJunction._options.transform, options);
    if ( !(this._options.inject_before && this._options.inject_after) )
      this._options.inject_after = true;
  }
  ...
  _transform(construct, encoding, callback) {
    ...
    if (this._options.match) {
      // do some filterin
      for (let [name, exp] of Object.entries(this._options.match)) {
        switch (exp.op) {
          case 'eq':
    ...
    if (forward && this._options.drop) {
      // do some filterin
      for (let [name, exp] of Object.entries(this._options.drop)) {
        switch (exp.op) {
          case 'eq':
            forward = forward && construct[name] && !(construct[name] === exp.value);
            break;
    ...
    if (forward) {
      ...
      if (this._options.inject_before)
        Object.assign(newConstruct, this._options.inject);

      if (this._options.mapping)   // JSON object with 'source': 'target' properties in dot notation
        dot.transform(this._options.mapping, construct, newConstruct);
      else
        Object.assign(newConstruct, construct);

      if (this._options.inject_after)
        Object.assign(newConstruct, this._options.inject);
