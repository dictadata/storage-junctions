...
module.exports = class CodifyWriter extends StorageWriter {
  ...
  constructor(storageJunction, options) {
    ...
    this._options = Object.assign({}, storageJunction._options.codify, options);

    // make a copy of junction's encoding
    this.engram = new Engram(this._engram.smt);
    ...
    if (this._options.statistics === true) {
  ...
  _write(construct, encoding, callback) {
    this._logger.debug("codify _write");

    try {
      // loop through construct fields
      this._processFields("", this.engram.fields, construct);

      //if (this._options.forEach)
      //  this._options.forEach(construct, this.engram, this._options);

    }
    catch (err) {
      this._logger.error("codify error", err);
    }
  ...
  _final(callback) {
    this._logger.debug("codify _final");

    // check if any fields are still undefined, i.e. all nulls; default to text
    let default_type = this._options.default_type || "text";
    this._checkDefaults(this.engram.fields, default_type);
   ...
   * @param {*} construct
   */
  _processFields(parent, fields, construct, dt = null) {
    this._logger.debug("processFields");
    ...
    for (let [name, value] of Object.entries(construct)) {
      // check field name
      let fldname = name;
      if (this._options.notation === "dot" && parent)
        fldname = parent + "." + name;
      ...
      if (stype === "null") {
        ...
        if (this._options.notation === "dot") {
          // keep it flat
          this._processFields(fldname, fields, value, dt);
        }
        else {
          // recurse sub-fields
        ...
        if (value.length > 0) {
          if (this._options.notation === "dot") {
            this._processFields(fldname, fields, value[0], dt);
          }
          else {
            if (!field.fields) field.fields = {};
            this._processFields("", field.fields, value[0], dt);
          }
        }
      }
      else {
  ...
  _checkDefaults(fields, default_type) {
    ...
    for (let field of Object.values(fields)) {
      if (field.type === "undefined")
        ...
        if (field.fields && (!this._options.notation || this._options.notation !== "dot"))
          this._checkDefaults(field.fields, default_type);  // recurse sub-fields
      }
    }
