...
module.exports = exports = class StorageJunction {
  ...
  async relax() {
    ...
    if (this._fileSystem)
      Cortex.FileSystems.relax(this._fileSystem);
    this._fileSystem = null;
  }
  ...
  async list(options) {
    ...
    // StorageJunctions that use FileSystem should implement the following.
  ...
  getReadStream(options) {
    ...
    if (reader.useTransforms) {
      if (options.match)
        reader = reader.pipe(this.getTransform("filter", {match: options.match}));
      if (options.fields)
        reader = reader.pipe(this.getTransform("select", {fields: options.fields}));
    }
    return reader;
  }
