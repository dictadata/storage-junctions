...
module.exports = exports = class StorageJunction {
  ...
  getReadStream(options) {
    ...
    if (reader.useTransforms && (options.match || options.cues && options.cues.fields)) {
      // source doesn't support queries, use filter and select transforms instead
      if (options.match) {
        let tr = this.getTransform("filter", {match: options.match});
        reader.pipe(tr);
        reader = tr;
      }
      if (options.cues && options.cues.fields) {
        let tr = this.getTransform("select", {fields: options.cues.fields});
        reader.pipe(tr);
        reader = tr;
      }
    }
  ...
  /**
   * Uses the prefix of smt.locus to determine the filestorage type.
   *
