...
module.exports = exports = class ElasticsearchJunction extends StorageJunction {
  ...
  async putEncoding(encoding) {
    ...
    try {
      ...
      // overwrite with any member _options
  ...
  async store(construct, pattern=null) {
    ...
    try {
      ...
      if (this._engram.keyof === 'uid' || this._engram.keyof === 'key') {
        // store by _id
        key = (pattern && pattern.key) || this._engram.get_uid(construct) || null;
      ...
      return new StorageResults(result, results.result, key, (this._options.meta ? results : null));
    }
    catch(err) {
      this._logger.error(err.statusCode, err.message);
      throw err;
  ...
  async recall(pattern) {
    ...
    try {
      if (this._engram.keyof === 'uid' || this._engram.keyof === 'key') {
        // get by _id
        let key = (pattern && pattern.key) || this._engram.get_uid(pattern) || null;
        if (!key)
        ...
        return new StorageResults(results.found ? 'ok' : 'not found', results._source, key, (this._options.meta ? results : null));
      }
      else if (this._engram.keyof === 'list' || this._engram.keyof === 'all') {
        // search by exact match
        let dsl = dslQuery.matchQuery(this._engram.keys, pattern);
        ...
        if (hits.length > 0)
          return new StorageResults("ok", hits[0]._source, null, (this._options.meta ? hits[0] : null) );
        else
  ...
  async dull(pattern = null) {
    ...
    try {
      ...
      return new StorageResults(result, null, null, (this._options.meta ? results : null));
    }
    catch(err) {
      this._logger.error(err.statusCode, err.message);
      throw err;
