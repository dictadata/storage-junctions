...
module.exports = class ElasticsearchJunction extends StorageJunction {
  ...
  async store(construct, pattern=null) {
    ...
    try {
      ...
      return new StorageResults(result, null, key, results);
    }
    catch(err) {
      this._logger.error(err.statusCode, err.message);
      throw err;
  ...
  async recall(pattern) {
    ...
    try {
      if (this._engram.keyof === 'uid' || this._engram.keyof === 'key') {
        ...
        if (hits.length > 0)
          return new StorageResults("ok", hits[0]._source, hits[0]._id, hits[0] );
        else 
  ...
  async retrieve(pattern = null) {
    ...
    try {
      ...
      if (!pattern.consolidate) {
        ...
        constructs = dslQuery.processAggregations(results.aggregations);
      }
