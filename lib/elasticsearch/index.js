...
module.exports = class ElasticsearchJunction extends StorageJunction {
   ...
   */
  async getEncoding() {
    //console.log("ElasticJunction getEncoding");
  ...
  async putEncoding(encoding) {
    //console.log("ElasticJunction putEncoding");
  ...
  */
  ...
  async store(construct, options = null) {
    //console.log("ElasticJunction store");

    try {
      let results = '';

      let uid = this._engram.get_uid(construct);
      if (uid)
        results = await this.elasticQuery.put(uid, construct);
      else
  ...
  async recall(options = null) {
    //console.log("ElasticJunction recall");

    let uid = (options && options.uid) || this._engram.uid;
    if (!uid)
      throw new StorageError({statusCode: 400}, "no storage uid specified");

    try {
      let results = await this.elasticQuery.get(uid);
  ...
  async dull(options = null) {
    //console.log("ElasticJunction dull");

    let uid = (options && options.uid) || this._engram.uid;
    if (!uid)
      throw "no storage uid specified";

    try {
      let results;
      if (uid)
        results = await this.elasticQuery.delete(uid);
      else
  ...
  async retrieve(pattern, options = null) {
    //console.log("ElasticJunction retrieve");
