...
module.exports = class ElasticTemplate {
  ...
  constructor(junction) {
    ...
    this.template_name = this.engram.schema;
    this.template = null;
    this.options = {
      node: this.engram.location,
      index: ''
    };
  ...
  async getEncoding() {
    ...
    for (let i = 0; i < names.length; i++) {
      ...
      if (name in exclude === false) {
        ...
        this.engram.encoding.add(field);
      }
    }
  ...
  async putEncoding(encoding, merge=false) {
    ...
    if (merge) {
      ...
      this.engram.encoding.fields = encoding.fields || encoding.encoding && encoding.encoding.fields;
