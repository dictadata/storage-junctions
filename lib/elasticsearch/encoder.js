...
let fieldsToMappings = exports.fieldsToMappings = function(fields) {
  ...
  for (let [name, field] of Object.entries(fields)) {
    ...
    if (ftype === "object") {
      mappings.properties[name] = fieldsToMappings(field);
    }
    else if (ftype === "array") {
      // get type from subtype
      if (!field.fields)
        throw new Error("invalid array subfield");

      let subfields = Object.values(field.fields);
      let subfield = subfields[0];
      mappings.properties[name] = { "type": elasticType(subfield) };

      if (subfield.default !== null && subfield.type !== 'text') {
        mappings.properties[name].null_value = subfield.default;
      }
