...
module.exports = async (junction) => {
  ...
  let options = junction._options;
  if (!options.scan)
    options.scan = {};

  // options needed by scanners
  if (!options.scan.filespec)
    options.scan.filespec = smt.schema;
...
async function fs_scan(dirpath, options) {
  ...
  try {
    ...
    for await (let dirent of dir) {
      if (dirent.isDirectory() && options.scan.recursive) {
        var subpath = dirpath + dirent.name; //path.join(dirpath, dirent.name);
        list.push(...await fs_scan(subpath, options));
      }
      else if (dirent.isFile() && dirent.name.match(rx)) {
        let fullpath = dirpath + dirent.name; //path.join(dirpath, dirent.name);
        list.push(fullpath);
        if (options.scan.forEach)
          await options.scan.forEach(fullpath);
      }
    }
    //await dir.close();
  }
    ...
    while (!done) {
      ...
      for (let entry of data.Contents) {
        ...
        if (prefix && entry.Key.indexOf(prefix) !== 0)
          continue;
        if (!options.scan.recursive && entry.Key.indexOf('/', prefix.length) >= 0)
          continue;
        if (entry.Key.match(rx)) {
          list.push(entry.Key);
          if (options.scan.forEach)
            await options.scan.forEach(entry.Key);
        }
      }
    }
  }
