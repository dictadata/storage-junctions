  ...
  async _read(size) {
    ...
    try {
      let axiosOptions = {
        ...
        headers: Object.assign({'Accept': 'application/json', 'User-Agent': '@dictadata.org/storage-node'}, this.junction.options.headers),
        auth: this.junction.options.auth || {},
        params: this.junction.options.params || {},
        timeout: this.junction.options.timeout || 10000
      };

      let url = this.junction.options.url || this.engram.smt.schema || '/';
