...
module.exports = exports = class Engrams {
  ...
  /**
   * @param { SMT }    smt an SMT string or SMT object where Codex Engrams data will be located. This parameter can NOT be an SMT name!
  ...
  /**
   * Activate the Codex Engrams junction
   *
  ...
  async activate() {
    ...
    try {
      ...
      if (results.status === 0) {
        logger.info("storage/codex/engrams: created schema, " + this._junction.smt.schema);
      }
      else if (results.status === 409) {
        logger.debug("storage/codex/engrams: schema exists");
      }
      else {
        throw new StorageError(500, "unable to create codex engrams schema");
      }
      this._active = true;
    }
    catch (err) {
      logger.error('storage/codex/engrams: activate junction failed, ', err.message || err);
    }
  ...
  /**
   *
   * @param {*} entry Engram or encoding object with Codex Entry properties
   * @returns
   */
  async store(entry) {
    ...
    if (this._junction) {
      ...
      logger.verbose("storage/codex/engrams: " + key + ", " + storageResults.status);
      return storageResults;
    }

    storageResults.setResults(500, "Codex Engrams junction not activated");
    return storageResults;
  }
  ...
  async recall(pattern) {
    ...
    if (this._engrams.has(key)) {
      ...
      // go to the engrams codex
      storageResults = await this._junction.recall({ key: key });
      logger.verbose("storage/codex/engrams: recall, " + storageResults.status);
    }
    else {
  ...
  async retrieve(pattern) {
    ...
    if (this._junction) {
      ...
      // retrieve list from engrams codex
      storageResults = await this._junction.retrieve(pattern);
      logger.verbose("storage/codex/engrams: retrieve, " + storageResults.status);
    }
    else {
      storageResults.setResults(503, "Codex Engrams Unavailable");
    }
